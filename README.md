...
Dateien
Datei	Inhalt
`classification\\\_report.txt`	Precision / Recall / F1 je Klasse + beste Hyperparameter
`confusion\\\_matrix.png` / `.csv`	Konfusionsmatrix über alle 35 Klassen
`partial\\\_deck\\\_accuracy.png` / `.csv`	Genauigkeit in Abhängigkeit vom Deck-Anteil
Einschränkungen
...
Regularisierung (`reg\\\_alpha`, `reg\\\_lambda`) gegen das Overfitting testen
